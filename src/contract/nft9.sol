...
//     Counters.Counter private _votecount;
...
//     uint256 public VOTING_DURATION = 1800;
...
//     mapping(uint256 => mapping(uint256 => nftBids)) public biddMap;
...
//     address[] public votingList; // List of addresses used in collectiondata
//     string[] public collectionList; // List of collection names used in collectiondata
//     address[] public adminList; // List of addresses used in collectiondata
...
//             uint256 bCount=bidCount[_tokenId];

//             biddMap[_tokenId][bCount] = nftBids({
//                 useraddress: msg.sender,
//                 bidAmount: msg.value,
//                 bidtime: block.timestamp
//             });

//             bidCount[_tokenId]=bCount+1;
...
//             tokenInfo.own = ownerNFT;
//             tokenInfo.seller = ownerNFT;
...
//         require(!compareStrings(collectiondata[msg.sender][_name].collection,_name), "Already Exist");
...
//     function getColl(address _add, uint8 _stt, string memory _cat, string memory _col) public view returns (Collections[] memory) {
//         uint256 addressCount = addressList.length;
//         uint256 collectionCount = collectionList.length;
//         uint256 maxItemCount = addressCount * collectionCount;
//         uint256 currentIndex = 0;

//         Collections[] memory tempItems = new Collections[](maxItemCount);
...
//                 if (_stt == 1 && currentItem.add != _add) {
//                     continue;
//                 }
//                 if (_stt == 2 && (currentItem.add != _add )) {
//                     continue;
//                 }else{
//                     if(_stt==2 && !compareStrings(currentItem.collection, _col)){continue;}
...
//     // Use a dynamic array to avoid over-allocation
//     tokenData[] memory tempItems = new tokenData[](unsoldItemCount);

//     for (uint256 i = 0; i < itemCount; i++) {
//         tokenData storage currentItem = tokenIDInfo[i + 1];
        
//         if (currentItem.status == statusOpen) {
//             tempItems[currentIndex] = currentItem;
//             currentIndex += 1;
//         }
//     }

//     // Create a fixed-size array from the temporary array
//     tokenData[] memory items = new tokenData[](currentIndex);
...
//                     continue;
//                 }
//                 if (_st == 2 && (tokenIDInfo[i + 1].seller != _add)) {
//                     continue;
//                 }
//                 else{
//                     if(_st==2 && !compareStrings((tokenIDInfo[i + 1].collection) , _col)){continue;}
//                     }
...
//      function getUserAllAuc(address _add) public view returns (tokenData[] memory) {
//         uint256 itemCount = _aucId.current();
//         uint256 currentIndex = 0;
...
//     //  function getwithId(uint256 _id) public view returns (tokenData[] memory) {
//     //     uint256 itemCount = _aucId.current();
//     //     uint256 currentIndex = 0;
...
//     //     for (uint256 i = 0; i < itemCount; i++) {
//     //         if (tokenIDInfo[i + 1].tokenId == _id) {
...
//     //     tokenData[] memory resultItems = new tokenData[](currentIndex);
//     //     // for (uint256 j = 0; j < currentIndex; j++) {
//     //         // resultItems = items[currentIndex-1];
//     //     // }
...
//     tokenData[] memory foundItem = new tokenData[](1); // Initialize an empty array
//     for (uint256 i = 0; i < itemCount; i++) {
//         if (tokenIDInfo[i + 1].tokenId == _id) {
//             tokenData storage currentItem = tokenIDInfo[i + 1];
//             if (currentItem.seller != address(0) && bytes(currentItem.collection).length != 0) {
//                // foundItem = new tokenData[] ; // Allocate array with one element
//                 foundItem[0] = currentItem;
//                 return foundItem;
//             }
//         }
//     }

//     // Return the empty array if the item is not found
//     return foundItem;
// }
...
//     // Use a dynamic array to avoid over-allocation
//     tokenData[] memory tempItems = new tokenData[](itemCount);
//     bytes32 colHash = keccak256(abi.encodePacked(_col));
//     bytes32 catHash = keccak256(abi.encodePacked(_cat));
...
//         if (currentItem.own == _add) {
//             if (_st == 1 && !compareStrings(currentItem.collection, _col)) {
//                 continue;
//             }
//             if (_st == 2 && (keccak256(abi.encodePacked(currentItem.collection)) != colHash || keccak256(abi.encodePacked(currentItem.category)) != catHash)) {
//                 continue;
//             }
//             if (currentItem.seller != address(0) && bytes(currentItem.collection).length != 0) {
//                 tempItems[currentIndex] = currentItem;
...
//     function getBidsForNFT(uint256 tokenId) public view returns (nftBids[] memory) {
//         uint256 count = bidCount[tokenId];
//         nftBids[] memory bids = new nftBids[](count);

//         uint256 index = 0;
//         for (uint256 i = 0; i < count; i++) {
//             nftBids memory bid = biddMap[tokenId][i];
//             if (bid.useraddress != address(0)) {
//                 bids[index] = bid;
//                 index++;
//             }
//         }

//         // Resize the array to remove empty entries if any
//         nftBids[] memory validBids = new nftBids[](index);
...
//     function createVoting() external payable{
//         require(msg.value>=listingPrice,"insufficient balance to create for creating profile");
//        // require(votings[msg.sender].celebrity==address(0),"Already applied for Profile");
//         require(votings[msg.sender].startTime == 0, "Voting already started");
//         require(block.timestamp>votings[msg.sender].lastClaim+1800,"after an hour you can applied for revoting");
//         address _celebrity=msg.sender;
//         votings[msg.sender] = Voting({
...
//         if (!addressExists(msg.sender)) {
//             votingList.push(msg.sender);
//         _votecount.increment();
//         }
//         if(msg.sender==ownerOfCon) 
//         {
//             votings[_celebrity].res=Result.Approved;
//             }
...
//         require(votings[msg.sender].res==Result.Approved || adminExists(msg.sender),"you are not celeberity or admin");
...
//     function adminvote(address _celebrity, bool _vote) payable external {
//         require(msg.value>=votingFee, "Pay voting fee");
//         require(adminExists(msg.sender), "you are not Admin");
//         require(votings[_celebrity].active, "Voting session Expired");
//         require(!hasVoted[msg.sender][_celebrity], "Already voted");
//         require(block.timestamp <= votings[_celebrity].startTime + VOTING_DURATION, "Voting ended");
...
//         votings[_celebrity].startTime =0;
...
//        votings[_celebrity].lastClaim=block.timestamp;
//         hasVoted[msg.sender][_celebrity] = false;
//         _votecount.decrement();
...
//     // function getAllActiveVotings() external view returns (Voting[] memory) {
...
//     // }
//     function getAllActiveVotings() public view returns (Voting[] memory) {
//         uint256 currentIndex = 0;
//         uint256 loop = votingList.length;

//         // Initialize an array with the maximum possible size
//         Voting[] memory items = new Voting[](loop);

//         for (uint256 i = 0; i < loop; i++) {
//             address currentAdd = votingList[i]; // Adjust index to access votingList correctly
//             Voting storage currentItem = votings[currentAdd];
            
//             if (currentItem.celebrity != address(0)) {
//                 items[currentIndex] = currentItem;
//                 currentIndex += 1;
//             }
//         }

//         // Resize the array to the actual number of active votings
//         Voting[] memory activeItems = new Voting[](currentIndex);
//         for (uint256 j = 0; j < currentIndex; j++) {
//             activeItems[j] = items[j];
//         }

//         return activeItems;
//     }

//     function setVotingDuration(uint256 _dur) public {
//         require(adminExists(msg.sender), "you are not Admin");
//         VOTING_DURATION=_dur;
//     }

//     function adminExists(address _addr) public view returns (bool) {
...
//     function setadmmin(address _add) public 
//     {
//         require(adminExists(msg.sender), "you are not Admin");
//         require(!adminExists(_add), "Already Exist");
//         adminList.push(_add);
//     }

//      function getAdminList() public view returns (address[] memory) {
//         address[] memory add = new address[](adminList.length);
//         for (uint i = 0; i < adminList.length; i++) {
//             add[i] = adminList[i];
//         }
//         return add;
